...
export class ProductService {
  ...
  private mapRowToEntity(row: Record<string, unknown>): Product {
    ...
    entity.category = row.category as Product['category'];
    entity.price = row.price as number;
    entity.cost = (row.cost as number | null) ?? 0;
    ...
    entity.minStock = (row.min_stock as number | null) ?? 0;
    entity.maxStock = (row.max_stock as number | null) ?? 0;
